                            ...
                            @foreach(parameter_get() as $no=>$param)
                                ...
                                @foreach(detail_parameter_get($param->id) as $no=>$detailparam)
                                    <tr>
                                        ...
                                        <td><b>{{$detailparam->name}} ({{$detailparam->nilai}}%)</b></td>
                                    ...
                                    @foreach(penilaian_get($detailparam->id) as $pen=>$penilaian)
                                        @if($penilaian->utama==1)
                                            <tr>
                                                ...
                                                <td><b>{{$penilaian->name}} ({{$penilaian->nilai}}%)</b></td>
                                            ...
                                            @foreach(detail_penilaian_get($penilaian->id) as $dtp=>$detpen)
                                                <tr>
                                                    ...
                                                    <td>{{$detpen->name}}</td>
                                                    <td>
                                                        <select name="jawaban_id" onchange="jawab(this.value,{{$data->id}},{{$detpen->id}},{{$detailparam->id}})">
                                                            <option value="">Pilih </option>
                                                            @foreach(jawaban_get(1) as $jawaban)
                                                            <option value="{{$jawaban->id}}" @if(jawaban_id($data->id,$detpen->id)==$jawaban->id) selected @endif>{{$jawaban->name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <td>{{nilai_id($data->id,$detpen->id)}}</td>
                                                    <td></td>
                                                    <td></td>
                                                </tr>
                                            @endforeach
                                        ...
                                        @if($penilaian->utama==3)
                                            <tr>
                                                ...
                                                <td>
                                                    <select name="jawaban_id" onchange="jawab(this.value,{{$data->id}},{{$penilaian->id}},{{$detailparam->id}})">
                                                        <option value="">Pilih</option>
                                                        @foreach(jawaban_get(1) as $jawaban)
                                                ...
                                                <td>{{nilai_id($data->id,$penilaian->id)}}</td>
                                                <td></td>
                                                <td></td>
                                            </tr>
                                        @endif
