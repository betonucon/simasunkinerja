...
use Validator;
use App\Lhe;
use App\Transaksikkesatu;
use App\Penilaian;
use App\Detailparameter;
use App\Penilaiankkesatu;
class TransaksikkesatuController extends Controller
{
    public function index(request $request){
        if(Auth::user()['role_id']==3){
            $menu='Laporan KKE1';
            $side='Transaksilke';
            return view('Transaksikkesatu.index_opd',compact('menu','side'));
        }
        if(Auth::user()['role_id']==2){
            $menu='Laporan KKE1';
            $side='Transaksilke';
            return view('Transaksikkesatu.index',compact('menu','side'));
        }
    ...
    public function create(request $request){
        if(Auth::user()['role_id']==3){
            $menu='Buat KKE1';
            $side='Transaksikkesatu';
            $data=Transaksikkesatu::where('id',base64_decode($request->id))->first();
            return view('Transaksikkesatu.view',compact('menu','side','data'));
        }
        if(Auth::user()['role_id']==2){
            $menu='Buat KKE1';
            $side='Transaksikkesatu';
            $data=Transaksikkesatu::where('id',base64_decode($request->id))->first();
            return view('Transaksikkesatu.create',compact('menu','side','data'));
        }
    ...
    public function saran(request $request){
        $data=Penilaiankkesatu::where('id',$request->id)->first();
        echo'
    ...
    public function save_kkesatu(request $request){
        error_reporting(0);
                    ...
                    if ($validator->fails()) {
                        ...
                        $cek=Penilaiankkesatu::where('transaksilhe_id',$request->transaksilhe_id)->where('penilaian_id',$request->id)->where('kategori',$request->kategori)->count();
                        if($cek>0){
                            $save=Penilaiankkesatu::where('transaksilhe_id',$request->transaksilhe_id)->where('penilaian_id',$request->id)->where('kategori',$request->kategori)->update([
    ...
    public function save_saran(request $request){
        error_reporting(0);
                    ...
                    $messages = [
                        'saran.required'=> 'Masukan saran/rekomendasi',
                    ];
                    $validator = Validator::make($request->all(), $rules, $messages);
                    $val=$validator->Errors();
                    ...
                    if ($validator->fails()) {
                        ...
                        echo'</div>';
                            ...
                            $save=Penilaiankkesatu::where('id',$request->id)->update([
                                'saran'=>$request->saran,
                            ]);
                            echo'ok';
